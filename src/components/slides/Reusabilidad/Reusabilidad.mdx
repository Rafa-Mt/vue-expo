...
Las composables pueden aceptar argumentos de ref o getter incluso si no confía en ellos para la reactividad. Si estás escribiendo un composable que puede ser utilizado por otros desarrolladores, es una buena idea manejar el caso de los argumentos de entrada que se están haciendo refs o getters en lugar de valores crudos. El toValue() es una función de utilidad  para este propósito:
...
```
  ...
  binding: Un objeto que contiene la información de la directiva.
    ...
    - component: El componente que contiene la directiva.
