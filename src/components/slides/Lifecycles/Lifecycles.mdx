...
import OnRenderT from "./OnRenderT.vue";
...
```
nos daria un resultado como este:



```html
Dependencia de renderizado rastreada: {effect: ReactiveEffect, target: RefImpl, type: 'get', key: 'value'}
```
...
<script setup>
  ...
  onRenderTriggered(event => {
    console.log('Dependencia de renderizado disparada:', event);
  });
</script>
...
```html	  
Dependencia de renderizado disparada: {type: "set", target: Proxy, key: "value", oldValue: 0, newValue: 1}
```
